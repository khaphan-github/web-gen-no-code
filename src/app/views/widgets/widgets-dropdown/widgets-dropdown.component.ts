...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
  ...
  public COUNT_SERVER_INFO: {
    API: number,
    TABLE: number,
    ROLE: number,
    ACCOUNT: number;
  } = { ACCOUNT: 0, API: 0, ROLE: 0, TABLE: 0 };

  ngOnInit(): void {
    this.setData();
    this.connectoServerEvent.getState().pipe(
      filter((event) => event?.fetchData),
      switchMap(() => this.getServerInfo())
    ).subscribe({
      next: (v) => {
        this.COUNT_SERVER_INFO = v;
      }
    })
  }
  ...
  getServerInfo() {
    return combineLatest([
      this.service.countGeneratedApi(),
      this.service.countTotalTable(),
      this.service.countRole(),
      this.service.countAccount(),
    ]).pipe(map(([api, table, role, account]) => {
      return {
        API: api?.data[0]?.count ?? 0,
        TABLE: table?.data[0]?.count ?? 0,
        ROLE: role?.data[0]?.count ?? 0,
        ACCOUNT: account?.data[0]?.count ?? 0,
      }
    }));
  }
